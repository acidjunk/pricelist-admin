...
const Shops = ({ shops = [], nb, translate, classes }) => (
                ...
                <GridList cols={3} className={classes.gridList}>
                    {shops.map(shop => (
                        <GridListTile key={shop.id}>
                            <img src={`https://www.prijslijst.info/static/uploaded/shops/${shop.id}.png`} alt={shop.name} />
                            <GridListTileBar
                                title={shop.name}
                                subtitle={<span>description: {shop.description}</span>}
                                actionIcon={
                                    <IconButton aria-label={`info about ${shop.name}`} className={classes.icon}>
                                        <InfoIcon />
                                    </IconButton>
                                }
                            />
    ...
    //                 >
    //                     <Avatar
    //                         src={`https://www.prijslijst.info/static/uploaded/shops/${record.id}.png`}
